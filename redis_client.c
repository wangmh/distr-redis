...
if (connection_entry->rh->err ){ \
    ...
    redis_log(L_ERR, "errstr is %s and retry %d %s:%d\n", connection_entry->rh->errstr,  retry, __FUNCTION__,\
                        __LINE__);\
   redisFree(connection_entry->rh); \
	...
	if(retry == 5){	\
		redis_log(L_ERR, "connection  err after try 5 times\n");\
...
static void initialize_redis_servers(redis_client_st *redis_client)
{
	...
	redis_client->redis_servers = g_hash_table_new_full(g_str_hash, g_str_equal, free_key,
			g_queue_free_all);
...
//初始化一致性hash算法
static void initialize_redis_ketama(redis_client_st * redis_client)
{
	int i;
	if (NULL == redis_client)
		return;
...
static char * get_hash_key(const char *key)
{
	if (key == NULL)
		return NULL;
...
static connection_pool_entry *get_redis_server_conn(redis_client_st *redis_client, const char *key)
{
	...
	if (hash_key)
	{
		...
		if (entry)
			redis_log(L_DEBUG, "key %s %s, %d fd:%d", hash_key, pool->host, pool->port,
					entry->rh->fd);
		else
...
redis_reply_st *redis_get(redis_client_st *redis_client, const char *key)
{
	...
	redis_reply_st *reply;
	reply = redisCommand(connection_entry->rh, "get %s", key);
	RECONNECTE(redisCommand(connection_entry->rh, "get %s", key));
...
{
	if (NULL == redis_client || NULL == key || NULL == value)
	{
		return NULL;
	}
	connection_pool_entry *connection_entry = get_redis_server_conn(redis_client, key);
	...
	redis_reply_st *reply;
	reply = redisCommand(connection_entry->rh, "set %s %s", key, value);
	RECONNECTE(redisCommand(connection_entry->rh, "set %s %s", key,value));
	redis_server_conn_pool_push(connection_entry);
	return reply;
}
...
redis_reply_st *redis_mget(redis_client_st *redis_client, char **keys, int len)
{
	...
	connection_pool_entry *connection_entry = get_redis_server_conn(redis_client, keys[0]);
	if (NULL == connection_entry)
	{
		return NULL;
	}
	...
	reply = redisCommandArgv(connection_entry->rh, len + 1, (const char **) argv, argvlen);
...
/****************************hash*****************************/
redis_reply_st *redis_hset(redis_client_st *redis_client, const char *key, const char *sub_key,
		const char *value)
{
	if (NULL == redis_client || NULL == key || NULL == sub_key || NULL == value)
	{
		return NULL;
	}
	connection_pool_entry *connection_entry = get_redis_server_conn(redis_client, key);
	...
	redis_reply_st *reply = NULL;
	reply = redisCommand(connection_entry->rh, "hset %s %s %s", key, sub_key, value);
	RECONNECTE(redisCommand(connection_entry->rh, "hset %s %s %s", key, sub_key, value));
	redis_server_conn_pool_push(connection_entry);
	return reply;
}
...
redis_reply_st *redis_hgetall(redis_client_st *redis_client, const char *key)
{
	...
	redis_reply_st *reply = NULL;
	reply = redisCommand(connection_entry->rh, "hgetall %s ", key);
	RECONNECTE(redisCommand(connection_entry->rh, "hgetall %s", key));
	redis_server_conn_pool_push(connection_entry);
	return reply;
}
...
{
	if (NULL == redis_client || NULL == key || NULL == sub_key)
	{
		return NULL;
	}
	connection_pool_entry *connection_entry = get_redis_server_conn(redis_client, key);
	...
	redis_reply_st *reply = NULL;
	reply = redisCommand(connection_entry->rh, "hget %s %s ", key, sub_key);
	RECONNECTE(redisCommand(connection_entry->rh, "hget %s %s ", key, sub_key));
	redis_server_conn_pool_push(connection_entry);
	return reply;
}
